...
import { changeInData } from "../Store/Reducers/DoctorReducer";

const DoctorPriofiePicUploadForm = ({ fn }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    cropper.getCroppedCanvas().toBlob(async (blob) => {
      ...
      formData.append("doctorPic", blob, file.name);
      formData.append("email", doctor.email);

      try {
        const res = await DoctorAxios.post("/doctor/picUpload", formData, {
          headers: {
  ...
  return (
    <div className="picModal w-full h-screen backdrop-blur-3xl z-50 px-4 fixed top-0 left-0">
      <form
        ...
        <button
          type="submit"
          className="bg-green-600 text-white uppercase text-lg w-[90%] px-20 py-3 rounded-sm">
          Upload
        </button>
      </form>
    </div>
...
export default DoctorPriofiePicUploadForm;
